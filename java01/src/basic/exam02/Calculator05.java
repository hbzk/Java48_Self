...
public class Calculator05 {
	 ...
	 * - 기본 생성자가 없으면 컴파일할 때 컴파일러가 자동으로 추가한다.
	 * - 인스턴스를 생성할 때 반드시 생성자가 호출된다.
	 * - 중요! 생성자의 존재 의의?
	 *  	인스턴스가 제대로 자신의 역할 수행할 수 있도록 최소한의 준비 시작하는것
	 *  => 유효한 초기값 할당.  
	 */
	public Calculator05() {}	// 아래에 생성자가 있기 때문에 기본 생성자가 자동으로 추가되지 않는다.
	
	/* 파라미터 값을 받는 생성자 선언
	 * - 인스턴스를 생성할 때 초기화 시킬 값을 생성자를 통해 받는다.
	 * - 이 생성자를 호출하기 위해서는 반드시 지정된 파라미터 값을 주어야 한다.
	 */
