...
public class Extest02 {
	...
	static class MyObject {
		// 예외를 보고하는 방법
		// - throw [Throwable 객체];
		// - 예외를 보고하는 메서드는, 반드시 메서드 선언부에 어떤 예외를 보고하는지 작성해야 함.
		...
		// 물론, 선언해도 상관없다.
		// RuntimeException Exception의 자식이다. (내 놓은 자식이다.
	...
	public static void main05(String[] args) {
		MyObject obj = new MyObject();
		// m(), my() 메서드는 Throwable, Exception 예외를 던지기 때문에
		// 반드시 try.. catch를 사용해야 한다.
		// => 프로그램을을하다보면 "메서드->메서드->메서드_메서드) 이렇게 여러단계에 걸쳐 메서드를 호출할 수 있다.
	...
	public static void main04(String[] args) {
  	...
  	// 각각의 예외에 대해 catch 블록을 여러 개 선언할 수 있다.
		// 단, 자식 예외 부터 받는 순서로 catch 블록을 만든다.

		try {
	    obj.m(); 		// Throwable
	    obj.m2();	// Exception
    } catch (Exception e) { 	// > 순서는 자식 예외 부터 선언
