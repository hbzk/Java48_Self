 ...
 * 		=> 애플리케이션을 종료했다가 다시 시작해도 기존에 작업한 데이터는 그대로 유지.
...
public class StudentMgtSystem {
	...
	private static void executeAdd(String value) {
	  ...
	  // >> 데이터를 추가 조작하기 쉽게 하기 위해, 배열을 새로운 인스턴스에 담아둔다.
	  StudentScore score = new StudentScore(data[0]);		// >> name을 파라미터로 보내면서 객체 생성
	  // Integer.parseInt() -> 인스턴스 없이 사용 가능하다면 클래스 메서드
