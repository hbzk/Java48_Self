...
/* JOIN ~ USING : 표준 문법 
조인 하려는 컬럼 이름이 같을 경우 사용 */
select *
from SE_USERS T1 join SE_STUDS T2
using (UNO);

/* JOIN ~ ON : 표준 문법 
조인 하려는 컬럼 이름이 다를 경우 사용 */
select *
from SE_USERS T1 join SE_STUDS T2
on T1.UNO = T2.UNO;

/* LEFT OUTER JOIN: 왼쪽을 기준으로 조인한다. 
왼쪽 테이블의 데이터는 모두 출력한다.*/
select T1.UNO, T1.NAME, T1.TEL, T2.COMP, T2.POS, T2.STAT
from SE_USERS T1 left outer join SE_STUDS T2
on T1.UNO = T2.UNO;

/* RIGHT OUTER JOIN: 오른쪽을 기준으로 조인한다. 
오른쪽 테이블의 데이터는 모두 출력한다.*/
select T1.UNO, T1.NAME, T1.TEL, T2.COMP, T2.POS, T2.STAT
