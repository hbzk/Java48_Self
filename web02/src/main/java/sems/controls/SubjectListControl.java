...
// @Servlet : 비지니스 로직을 수행하는 서비스 역할 객체에게 붙인다.
// @Repository : DAO 객체에 붙인다.
// 객체의 역할별로 애노테이션을 붙이면, 클래스의 역할을 빠르게 파악 할 수 있어서 좋다.
...
public class SubjectListControl {
	...
	// 1) 호출 될 메서드는 @RequestMapping으로 선언한다.
	// 2) 파라미터 값을 꺼내야 한다면, 메서드의 파라미터로 선언한다.
	//		- 문자열이 기본 타입(byte, short, int, long, float, double, boolean, char)으로 자동 변환된다.
